	...
	<section class="top-bar-section">
		...
		<ul class="left">
			...
			<li class="<% if $LinkingMode == "current" || $LinkingMode == "section" %>active<% end_if %><% if $Children %> has-dropdown<% end_if %>">
				<a href="$Link" <% if $ID == 2405 %>style="font-size:12px;"<% end_if %><% if $URLSegment == "feedback" %>data-reveal-id="{$URLSegment}-form-modal"<% end_if %> title="Go to the $Title.ATT">$MenuTitle</a>
				<% if $Children %>
				<ul class="dropdown">
